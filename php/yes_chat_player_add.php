...
catch(PDOException $e) 
{
 ...
 //отправляет запрос на добавление игрока в чат(точнее изменение статуса игрока на [простой участник])
//$sth = $dbh->query('SELECT * FROM myDB.list_chats where');
$str=$_GET['php_player'];
$str1=$_GET['php_chat'];
$str2 ='member';
$sth = $dbh->prepare('update myDB.list_chats set myDB.list_chats.status=:pr_status where (list_chats.player=:pr_player and myDB.list_chats.chat=:pr_chat)');
